...
draw.text((75, 98), "Supplement Protocol  —  Week 1", font=font_sub, fill=light_gray)
...
morning = [
    ...
    ("Retatrutide", "60 units SubQ", "Once per week · 5mg vial", gold, "WEEKLY"),
    ("Selank", "18 units SubQ", "Daily · 5mg vial · Cognitive support", blue, "DAILY · AM"),
    ("MOTS-C", "10 units  (1mg)", "Daily · 5mg vial · Morning or pre-workout", orange, "DAILY · PRE-WO"),
    ("Kisspeptin", "5 units SubQ", "Daily · 10mg vial · Hormonal support", pink, "DAILY · AM"),
    ("Instant Hydration", "1 packet in 16oz water", "500mg Na · 470mg K · 100mg Mg · Zero sugar", green, "ELECTROLYTES"),
]

for name, dose, note, color, tag in morning:
    ...
    idx = morning.index((name, dose, note, color, tag)) + 1
    draw.text((50, y+18), str(idx), font=font_label, fill=gold)
...
for name, dose, note, color, tag in food:
    rrect(draw, [25, y, W-25, y+100], 16, dark_card, outline=divider, w=1)
    rrect(draw, [25, y, 32, y+100], 4, color)
    idx2 = food.index((name, dose, note, color, tag)) + 1
    rrect(draw, [38, y+12, 82, y+60], 20, accent)
    draw.text((50, y+18), str(idx2), font=font_label, fill=gold)
    draw.text((92, y+14), name, font=font_label, fill=white)
    draw.text((92, y+52), note, font=font_tiny, fill="#666680")
    tag_w = len(tag) * 12 + 20
    tx = W - tag_w - 28
...
night = [
    ("DSIP", "10 units SubQ", "Daily · 5mg vial · Deep sleep support", purple, "DAILY · PM"),
    ("Magnesium Glycinate", "Per label dose", "With water · Muscle & nerve recovery", purple, "RECOVERY"),
]

for name, dose, note, color, tag in night:
    rrect(draw, [25, y, W-25, y+100], 16, dark_card, outline=divider, w=1)
    rrect(draw, [25, y, 32, y+100], 4, color)
    idx3 = night.index((name, dose, note, color, tag)) + 1
    rrect(draw, [38, y+12, 82, y+60], 20, accent)
    draw.text((50, y+18), str(idx3), font=font_label, fill=gold)
    draw.text((92, y+14), name, font=font_label, fill=white)
    draw.text((92, y+52), note, font=font_tiny, fill="#666680")
    tag_w = len(tag) * 12 + 20
    tx = W - tag_w - 28
...
# Footer
y += 10
rrect(draw, [25, y, W-25, y+140], 16, "#0D0D1A", outline=divider, w=1)
draw.text((50, y+14), "💉  All peptides with 100-unit insulin syringe · SubQ injection", font=font_small, fill=light_gray)
draw.text((50, y+44), "❄️   Refrigerate all peptides after reconstitution", font=font_small, fill=light_gray)
draw.text((50, y+74), "📅  Retatrutide once per week — same day every week", font=font_small, fill=gold)
draw.text((50, y+104), "⚠️   For personal use only — follow your prescribed protocol", font=font_tiny, fill="#444455")

img.save("/app/jhonnatan_weekly_protocol.png", quality=98)
print("Done")
